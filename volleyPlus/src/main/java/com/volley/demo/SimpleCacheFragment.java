...
/**
 * The main fragment that powers the ImageGridActivity screen. Fairly
 * straight
 * forward GridView implementation with the key addition being the
 * ImageWorker
 * class w/ImageCache to load children asynchronously, keeping the UI nice
 * and
 * smooth and caching thumbnails for quick retrieval. The cache is retained
 * over
 * configuration changes like orientation change so the images are
 * populated
...
public class SimpleCacheFragment extends Fragment implements AdapterView.OnItemClickListener {
                        ...
                        if (mAdapter.getNumColumns() == 0) {
                            ...
                            if (numColumns > 0) {
                                ...
                                if (Utils.hasJellyBean()) {
                                    mGridView.getViewTreeObserver().removeOnGlobalLayoutListener(
                                            this);
                                } else {
    ...
    /**
     * The main adapter that backs the GridView. This is fairly standard
     * except
     * the number of columns in the GridView is used to create a fake top
     * row of
     * empty views as we use a transparent ActionBar and don't want the
     * real top
    ...
    private class ImageAdapter extends BaseAdapter {
        ...
        /**
         * Sets the item height. Useful for when we know the column width
         * so the
        ...
        public void setItemHeight(int height) {
            ...
            mImageViewLayoutParams = new GridView.LayoutParams(LayoutParams.MATCH_PARENT,
                    mItemHeight);
            mImageLoader.setMaxImageSize(height);
